...
- `KolibriRuntime` после каждого запроса собирает отчёт `mksi_snapshot` с
  текущими и скользящими значениями осей G/P/A/R/E/U. Событие автоматически
  попадает в `ActionJournal`, рядом с `slo_snapshot`.
- Для экспорта метрик укажите параметры конструктора `KolibriRuntime`:
  - `mksi_export_file="logs/mksi.json"` — запись JSON-файла на диске.
  - `mksi_http_endpoint="http://localhost:8080/hooks/mksi"` — HTTP POST с
    payload `{"current": ..., "rolling": ...}`.
- Журналы удобно просматривать через команду `:journal` в CLI: последний
  элемент покажет mKSI и позволит сопоставить его со стадиями пайплайна.

### Подключение к CI

1. В тестовом раннере инициализируйте runtime с путём экспорта, чтобы получить
   артефакт после прогона миссий.
2. Добавьте проверку: `rolling.mksi` не ниже целевого порога и каждая ось
   соответствует требованиям текущего канала релиза.
3. Публикуйте JSON и action-journal как артефакты пайплайна, чтобы команда могла
   анализировать деградации без повторного запуска.
