...
static bool parse_line(const char* line, ReasonBlock* b){
    if(!line || !b){
        return false;
    }
    memset(b, 0, sizeof(*b));
    ...
    p=strstr(line,"\"origin_node\":\"");
    if(p){
        p+=strlen("\"origin_node\":\"");
        size_t j=0;
        while(*p && *p!='"' && j<sizeof(b->origin_node)-1){
            if(*p=='\\' && p[1]) p++;
            b->origin_node[j++]=*p++;
        }
        b->origin_node[j]=0;
...
bool chain_validate_block(const ReasonBlock* b, const kolibri_config_t* cfg, const char* expected_prev_hash){
    if(!b){
        return false;
    }
    if(expected_prev_hash && strncmp(expected_prev_hash, b->prev, 64) != 0){
        return false;
    }
    char payload[4096];
    int m = rb_payload_json(b, payload, sizeof(payload));
    if(m < 0){
        return false;
    }
    char h[65];
...
bool chain_get_summary(const char* path, kolibri_chain_summary_t* out, const kolibri_config_t* cfg){
    ...
    char line[8192];
    char prev_hash[65] = {0};
    kolibri_chain_summary_t summary = {0};
    ReasonBlock b;
    while(fgets(line, sizeof(line), f)){
        ...
        if(!parse_line(line, &b)){
            fclose(f);
            return false;
        }
        if(cfg && !chain_validate_block(&b, cfg, prev_hash)){
            fclose(f);
            return false;
        }
        snprintf(prev_hash, sizeof(prev_hash), "%s", b.hash);
        summary.height = b.step;
        snprintf(summary.head_hash, sizeof(summary.head_hash), "%s", b.hash);
        snprintf(summary.prev_hash, sizeof(summary.prev_hash), "%s", b.prev);
        snprintf(summary.fingerprint, sizeof(summary.fingerprint), "%s", b.config_fingerprint);
    }
    fclose(f);
    if(out){
        *out = summary;
